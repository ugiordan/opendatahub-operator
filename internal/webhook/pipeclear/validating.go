...
func (v *Validator) validatePipelineSpec(ctx context.Context, spec map[string]interface{}) admission.Response {
	...
	for executorName, executorRaw := range executors {
		...
		// Check for disallowed registries (example: docker.io for production)
		if strings.HasPrefix(image, "docker.io/") {
			warnings = append(warnings, fmt.Sprintf("image %q uses docker.io - consider using a Red Hat certified registry", image))
		}
	}
