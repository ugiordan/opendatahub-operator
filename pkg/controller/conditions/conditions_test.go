...
func TestManager_TimestampUpdatedWhenConditionChanges(t *testing.T) {
	g := NewWithT(t)
	...
	updatedCondition := manager.GetCondition(dependency1Condition)
	g.Expect(updatedCondition).NotTo(BeNil())
	g.Expect(updatedCondition.Status).To(Equal(metav1.ConditionFalse))
	g.Expect(updatedCondition.LastTransitionTime).NotTo(Equal(originalTime))
}
