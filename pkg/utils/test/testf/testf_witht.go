	...
	return &EventuallyValue[*unstructured.Unstructured]{
		...
		f: func(ctx context.Context) (*unstructured.Unstructured, error) {
			...
			case errors.IsForbidden(err):
				return nil, StopErr(err, "failed to create or update resource: %s, nn: %s", gvk, nn.String())
			case err != nil:
				return nil, err
			default:
				return obj, nil
...
func (t *WithT) CreateOrPatch(
	...
	return &EventuallyValue[*unstructured.Unstructured]{
		...
		f: func(ctx context.Context) (*unstructured.Unstructured, error) {
			...
			_, err := controllerutil.CreateOrPatch(ctx, t.Client(), obj, func() error {
				return fn(obj)
			})
			...
			switch {
			case err != nil:
				return nil, StopErr(err, "failed to create or patch resource: %s, nn: %s", gvk, nn.String())
			default:
